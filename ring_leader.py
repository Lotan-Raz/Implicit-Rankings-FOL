...
def ring_leader_with_messages():
    
    print('ring_leader_with_messages')
    ...
    ###WE CAN NOW ALSO CHECK THAT THIS REALLY DEFINES A RANKING FUNCTION:
    background_remains = lambda sym1,sym2: And(
        ForAll([X,Y,Z],sym2['btw'](X,Y,Z)==sym1['btw'](X,Y,Z)),
        ForAll([A,B],sym2['le'](A,B)==sym1['le'](A,B)),
        ForAll(X,sym2['id'](X)==sym1['id'](X)),
        sym2['max']==sym1['max'],
        axiom(sym1),axiom(sym2)
    )
    order = lambda sym1,sym2: And(rank1.reduced(sym1,sym2),background_remains(sym1,sym2))
    #formula_defines_order(order,[Node],constant_sym,relation_sym,function_sym)

ring_leader_with_messages()

def ring_leader_without_messages():
    
    #This is quite a model which is quite less natural, from the regular abstractions paper
