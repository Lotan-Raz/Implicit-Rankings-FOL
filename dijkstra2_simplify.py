...
def dijsktra2():
    ...
    print('dijkstra2_instrumented_try_simple')
    ...
    #the following two formulas assert that the instrumentation is correct (as long as we do have next and prev candidates, which is faulty in infinite models)
    #we may be able to assert these as invariants, but probably will be trouble for the solver,
    #we must assume them in init to guarantee that the instrumentation variables are initiated to correct values.
    def priv_above_inst_is_correct(sym):
        return And(ForAll(X,Implies(X!=sym['top'],sym['priv_above'](X)==ForAll(Y,Implies(succ_node(sym,X,Y),
                    And(sym['x'](X)==sym['x'](Y),sym['up'](X),Not(sym['up'](Y))))))),
                sym['priv_above'](sym['top'])==False,
        )
    
    def priv_bellow_inst_is_correct(sym):
        return And(ForAll(X,Implies(X!=sym['bot'],sym['priv_below'](X)==ForAll(Y,Implies(succ_node(sym,Y,X),
                    sym['x'](X)!=sym['x'](Y))))),
                sym['priv_below'](sym['bot'])==False,
        )
    ...
    #This is the exact same ranking we use in two token stabilization
    ...
    num_privs = ParPermute2FreeRank(binary_token_xt,param_xt,{},
                                       hints_A,hints_A,hints_B)
    proof_simple = LivenessProof(prop,num_privs,rho,phi,[psi])
    #proof_simple.check_proof(ts) #this works for conserved, with enough instrumentation axioms
    ...
    sum_distances = ParPermute2FreeRank(token_xt_and_distance_xt,param_xt,{},
                                          hints_A,hints_A,hints_B)
    rank = LexFreeRank([num_privs,sum_distances])

    proof = LivenessProof(prop,rank,rho,phi,[psi])
    
    #proof.check_proof(ts) #succeeds ~68 sec



    #For some reason does not work

    def predicate_xti(sym,param):
        #x is free
        #i is the node index
        #t is the type of move 
        x = param['x']
        return Or(
            And(priv_below(sym,x),
                Or(
                    And(
                        #z-x / n-1-x 
                        param['ty']==below, #THIS HAS DIFFERENT MEANING THAN ABOVE
                        sym['leq_node'](param['x'],param['i']),
                        ForAll(Y,Implies(And(sym['leq_node'](param['x'],Y),sym['leq_node'](Y,param['i']),param['i']!=Y),
                                        Not(priv_by_type(sym,Y,above)))), 
                    ),
    ...
    hints_it = [i_to_x | t_to_above, i_to_skd | t_to_above,
                i_to_x | t_to_below, i_to_skd | t_to_below]
    binary_xti = BinaryFreeRank(predicate_xti,param_xti)
    sum_it = ParPointwiseFreeRank(binary_xti,{'i':Node,'ty':PrivType},{'x':Node},hints_it)
    sum_x = ParPermFreeRank(sum_it,{'x':Node},{},1,[x_to_skd],[x_to_skd,x_to_prev,x_to_next])

    rank_variant = LexFreeRank([num_privs,sum_x])

    proof_variant = LivenessProof(prop,rank_variant,rho,phi,[psi])    

    start_time = time.time()
    proof_variant.premise_conserved(ts) 
    end_time = time.time()
