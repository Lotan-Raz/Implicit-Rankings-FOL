...
def dijsktra2():
    ...
    #This is a version with a simplification, which we might be able to remove later,
    #instead of defining the two privileges as derived relations, we use insturmentation and rewrites,
    #the goal is to remove the necessity for the next(x) and prev(x) fields
    #for this purpose we add variables next_skd, prev_skd axiomitized to be the next and prev of skd respectively
    #there is yet some more to be said about exactly why this instrumentation is sound.

    print('dijkstra2_instrumented')
    ...
    cond1 = lambda sym,param: Exists(X,And(sym['leq_node'](sym['node_h'],X),priv_below(sym,X)))
    cond2 = lambda sym,param: Exists(X,priv_above(sym,X))
    cond3 = lambda sym,param: Exists(X,And(sym['leq_node'](X,sym['node_h']),priv_below(sym,X)))
    delta_lin = LinFreeRank([rank_below_right,rank_above_left,rank_below_right],[cond1,cond2,cond3])
