...
#we can consider the simplification they use where you just assumue every transition is called infinitely (and no drops?)

#receiver_scheduled -> ack_sent so actually the fourth fairness reduces to GF ack_received
#and similarly for the third although its slightly more complicated probably
#but this should be part of the reasoning to be done.

timer_system = timer_transition_system(formula,{'sk_index':Index,'bot':Value,'sender_scheduled':BoolSort(),'receiver_scheduled':BoolSort(),
...
def proof_old():
    ...
    def timer_invariant(sym):
        return And(
            ...
            Or(
                sym['t_<G(Not(data_sent))>']>=0, #this needs to be part  of the rank
                sym['t_<G(F(data_received))>']==0
            ),
            Or(
                sym['t_<G(Not(ack_sent))>']>=0, #this needs to be part  of the rank
